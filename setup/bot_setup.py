...
async def create_active_client():  
    ...
    SP.print_status("info", 4, f"Checking if client path exists")
    if os.path.exists(client_path):
        SP.print_status("info", 4, f"Checking for client module")
        file, pathname, description = imp.find_module(name="client", path=[client_path])
        SP.print_status("info", 4, f"Loading client module")
        client = imp.load_module("client", file, pathname, description)
        SP.print_status("success", 5, f"Loded client module")
    else:
        SP.print_status("error", 1, f"Client [{active_client_name}] doesn't exist. Check the config at res/config.json\nPath: {client_path}")
        exit()
    token = dotenv_handler.get_config_token()
    SP.print_status("info", 4, f"Instantiating client")
    active_client = client.Client(token)
    return active_client

async def load_all_plugins(client : discord.Client):
    plugin_path = f"{client_path}/plugins"
    SP.print_status("info", 4, f"Loading all bot Plugins")
